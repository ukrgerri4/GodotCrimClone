...
func update_waves():
	...
	for _i in range(NUMBER_OF_WAVES):
		var _wavelength = random.randf_range(wavelength/2.0, wavelength*2.0)
		var _wind_direction = wind_direction.rotated(random.randf_range(-PI, PI)*(1-wind_align))
		
		waves.append({
			'amplitude': amp_length_ratio * _wavelength,
			'steepness': random.randf_range(0, steepness),
